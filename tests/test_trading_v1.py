...
def _add_entry_outcome(state, idx, ret_pct, cluster="trend", category=None,
    ...
    # 1-second spacing: keeps threshold tests exact under Learning v3's
    # 14d half-life decay (day spacing would decay counts between events).
    event = record_entry_event(
        state, cand, decision, "test", ts=1_700_000_000 + idx,
        regime="neutral",
    )
...
class TradingV2AttributionTests(unittest.TestCase):
    ...
    def test_exit_profile_activates_after_sixty_samples(self):
        ...
        record_exit_event(state, "M60", holding, "trail", 4.0, 104.0,
                          ts=1_700_100_000 + EXIT_LIVE_N, regime="neutral")
    ...
    def test_record_entry_event_records_skips_but_ignores_other_decisions(self):
        """Learning v3: skipped candidates ARE recorded (learn from omission);
        anything that is neither executed nor skipped stays ignored."""
        state = {}
        event = record_entry_event(
            state,
        ...
        self.assertFalse(event["was_executed"])
        self.assertEqual(event["skip_reason"], "EV gate")
        self.assertEqual(len(state.get("attribution_events")), 1)
        ignored = record_entry_event(
            state,
            _entry_candidate("REJ", cluster="trend"),
            "rejected",
            "hard block",
            ts=124,
            regime="neutral",
